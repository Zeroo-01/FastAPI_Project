...
def get_posts(db: Session = Depends(get_db),current_user: int = Depends(oauth2.get_current_user),limit:int=10,skip:int=0,search: Optional[str]=""):
    ...
    # posts=db.query(models.Post).filter(models.Post.owner_id==current_user.id).all()  for getting post of the current user only
...
def createpost(post: schemas.PostCreate,db: Session = Depends(get_db),current_user: int  = Depends(oauth2.get_current_user)):
    ...
    new_post=models.Post(owner_id=current_user.id,**post.model_dump())  #model_dump instead of dict
    # new_post=models.Post(title=post.title,content=post.content,published=post.published) unpacked post.dict for the same result
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return new_post 
